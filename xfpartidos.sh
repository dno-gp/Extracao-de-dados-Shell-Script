...
for x in $(cat partidos.txt); do #Substituir o aquivo txt por uma array/lista
	wget http://agencia.tse.jus.br/estatistica/sead/eleitorado/filiados/uf/filiados_${x}_pb.zip
...
for x in $(cat ../munics.txt); do
	grep -iw "$x" auxiliar.txt >> partidos_ms.txt #Filtra conforme lista de municipios estabelecidos
done
